...
# Logging
# -------------------------
$script:LogFolder  = Join-Path (Get-Location).Path "MobilityAgentInstallAutomationLogs"
$script:LogFilePath = $null

function Initialize-Logging {
    <#
    .SYNOPSIS
    Starts transcript-based logging to a timestamped log file.
    All Write-Host, Write-Warning, and Write-Error output is captured automatically.

    .PARAMETER LogFolder
    Folder where log files are stored. Defaults to .\MobilityAgentInstallAutomationLogs.

    .PARAMETER HostName
    Used in the log file name for easy identification.
    #>
    [CmdletBinding()]
    param(
        [string] $LogFolder = $script:LogFolder,
        [string] $HostName  = "batch"
    )
    ...
    $script:LogFilePath = Join-Path $LogFolder "Install-MobilityAgent_${safeHost}_${timestamp}.log"
    ...
    Write-Host "[Logging] Log file : $($script:LogFilePath)" -ForegroundColor Cyan
    Write-Host "[Logging] Started  : $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" -ForegroundColor Cyan
    Write-Host ""
...
function Complete-Logging {
    <#
    .SYNOPSIS
    Stops transcript logging and prints the log file location.
    #>
    [CmdletBinding()]
    param()
    ...
    if ($script:LogFilePath) {
        Write-Host ""
        Write-Host "[Logging] Completed : $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" -ForegroundColor Cyan
        Write-Host "[Logging] Log saved : $($script:LogFilePath)" -ForegroundColor Cyan
...
function Invoke-LinuxSshCommand {
    ...
    param(
        ...
        [int]    $TimeoutSec = 0,  # 0 = no timeout; overall command timeout in seconds
    ...
    # Use async output reading to avoid deadlocks and support timeout
    $stdoutBuilder = New-Object System.Text.StringBuilder
    $stderrBuilder = New-Object System.Text.StringBuilder

    $stdoutEvent = Register-ObjectEvent -InputObject $p -EventName OutputDataReceived -Action {
        if ($null -ne $EventArgs.Data) {
            [void]$Event.MessageData.AppendLine($EventArgs.Data)
        }
    } -MessageData $stdoutBuilder

    $stderrEvent = Register-ObjectEvent -InputObject $p -EventName ErrorDataReceived -Action {
        if ($null -ne $EventArgs.Data) {
            [void]$Event.MessageData.AppendLine($EventArgs.Data)
        }
    ...
    if ($TimeoutSec -gt 0) {
        ...
        if (-not $exited) {
            $timedOut = $true
            try { $p.Kill() } catch { }
            # Give the process a moment to flush output after kill
    ...
    # Small delay to let event handlers finish flushing
...
function Invoke-RemoteInstaller {
    ...
    .PARAMETER InstallerTimeoutSec
    Timeout in seconds for the installer execution step. Default is 1800 (30 minutes).
    If the installer does not complete within this time, the SSH session is terminated
    and the script checks the remote install process status before failing.
    ...
    param(
        ...
        [Parameter(Mandatory)] [string] $RemoteWorkFolder,
        [int] $Port = 22,
        [int] $InstallerTimeoutSec = 1800  # 30 minutes default
    )
    ...
    if ($InstallerTimeoutSec -gt 0) {
        Write-Host "[Run Installer] Executing installer (timeout: $InstallerTimeoutSec seconds)..." -ForegroundColor Gray
    }
    else {
        Write-Host "[Run Installer] Executing installer (no timeout)..." -ForegroundColor Gray
    }
    # Redirect installer stdout/stderr to a log file, then cat it afterward.
    # This prevents child processes (services started by the installer) from
    # inheriting the SSH session's stdout/stderr pipe file descriptors, which
    # would cause the SSH session to hang after the installer completes.
    # See: https://unix.stackexchange.com/questions/30400
    $installLogPath = "$RemoteWorkFolder/install_output.log"
    $installCmd = "cd $RemoteWorkFolder && ./install -r MS -v VmWare -q -c CSPrime >$installLogPath 2>&1; INSTALLER_RC=`$?; cat $installLogPath; (exit `$INSTALLER_RC)"
    ...
    # Handle timeout: check if the install process is still running on the remote machine
    if ($installResult.TimedOut) {
        Write-Host "[Run Installer] [TIMEOUT] Installer did not complete within $InstallerTimeoutSec seconds" -ForegroundColor Yellow
        Write-Host "[Run Installer] Partial STDOUT: $($installResult.StdOut)" -ForegroundColor Gray
        Write-Host "[Run Installer] Checking remote installer process status..." -ForegroundColor Yellow
        
        try {
            # Check if the install process is still running on the remote host
            $statusCmd = "ps aux | grep -E './install\s+-r' | grep -v grep"
            $statusResult = Invoke-LinuxSshCommand `
                -HostName $HostName `
                -UserName $UserName `
                -Command $statusCmd `
                -Port $Port `
            ...
            if (-not [string]::IsNullOrWhiteSpace($statusResult.StdOut)) {
                Write-Host "[Run Installer] [INFO] Installer process is still running on $HostName" -ForegroundColor Yellow
                Write-Host "[Run Installer] Remote process info:" -ForegroundColor Gray
                Write-Host $statusResult.StdOut -ForegroundColor Gray
                throw "Installer timed out after $InstallerTimeoutSec seconds and is still running on $HostName. " +
                      "The install process may complete on its own. Check manually with: ssh $UserName@$HostName 'ps aux | grep install'"
            }
            else {
                Write-Host "[Run Installer] [INFO] Installer process is no longer running on $HostName" -ForegroundColor Yellow
                
                # The process finished but we timed out reading the SSH output.
                # Check if the agent binary exists as a proxy for success.
                $verifyCmd = "test -f /usr/local/ASR/Vx/bin/AzureRcmCli && echo 'AGENT_FOUND' || echo 'AGENT_NOT_FOUND'"
                $verifyResult = Invoke-LinuxSshCommand `
                    -HostName $HostName `
                    -UserName $UserName `
                    -Command $verifyCmd `
                ...
                if ($verifyResult.StdOut -match 'AGENT_FOUND') {
                    Write-Host "[Run Installer] [OK] Agent binary found - installer likely completed successfully despite SSH timeout" -ForegroundColor Green
                    Write-Host ""
                    ...
                    return [pscustomobject]@{
                        ...
                        SshClientExit  = -1
                        RemoteExitCode = 0
                        StdOut         = $installResult.StdOut + "`n[NOTE: SSH timed out but agent binary was found]"
                        StdErr         = $installResult.StdErr
                    }
                }
                else {
                    throw "Installer timed out after $InstallerTimeoutSec seconds. Process is no longer running and agent binary was not found. Installation may have failed."
                }
            }
        }
        catch [System.Management.Automation.RuntimeException] {
            # Re-throw our own throw statements
            throw
        }
        catch {
            throw "Installer timed out after $InstallerTimeoutSec seconds and status check also failed: $_"
        }
    }
...
function Invoke-RemoteConfiguration {
    ...
    $configJsonRemotePath = "$RemoteWorkFolder/config.json"
    $applyConfigCmd = "/usr/local/ASR/Vx/bin/UnifiedAgentConfigurator.sh -q -c CSPrime -D true -S $configJsonRemotePath"
...
function Install-MobilityAgent {
    ...
    .PARAMETER InstallerTimeoutSec
    Timeout in seconds for the remote installer execution. Default is 1800 (30 minutes).
    If the installer hangs, the script will kill the SSH session, check the remote process
    status, and report whether the install is still running. Set to 0 for no timeout.
    ...
    param(
        ...
        [int] $InstallerTimeoutSec = 1800  # 30 minutes default; 0 = no timeout
    )
    ...
    # Initialize logging
    Initialize-Logging -HostName $HostName
    ...
    # Run installer
    try {
        Invoke-RemoteInstaller -HostName $HostName -UserName $UserName -RemoteWorkFolder $remoteWorkFolder -Port $Port -InstallerTimeoutSec $InstallerTimeoutSec | Out-Null
    }
    catch {
...
function Install-MobilityAgentFromFile {
    ...
    # Initialize logging for batch run
    Initialize-Logging -HostName "batch"
